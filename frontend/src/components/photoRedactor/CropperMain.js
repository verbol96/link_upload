...
const CropperMain = ({ photos, setPhotos, onSaveCrop, handleAddPhotos, setActivePhoto, activePhoto, savePhotos, nameOrder, setNameOrder, setNameFormat,
    ...
    const minZoom = (currentFieldsWhite) => {
        ...
        if (currentFieldsWhite) {
            // С полями — минимум = фото вписано по меньшей стороне
            return Math.min(
                cropSize.width  / effectiveMediaW,
                cropSize.height / effectiveMediaH,
            );
        } else {
            // Без полей — минимум = фото заполняет (без полей)
            return Math.max(
                cropSize.width  / effectiveMediaW,
                cropSize.height / effectiveMediaH,
            );
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            if (e.key === 'ArrowLeft' && activePhoto > 0) {
                ...
                e.preventDefault();
                RotationImg(-1);
            } else if (e.key === 'ArrowUp' && activePhoto < photos.length - 1) {
                e.preventDefault();
                RotationAspect();
            }
        };
    ...
    }, [activePhoto, photos.length, changePhoto, addField]);
                        ...
                        <div className="w-full flex flex-col justify-between h-full my-2 md:my-4 px-2 md:px-0">
                          ...
                          {/* МОБИЛЬНАЯ ПАНЕЛЬ: 3 кнопки + свайп-зона */}
                            <div className="md:hidden w-full flex flex-col gap-3">
                                ...
                                {/* Свайп-зона с точками-индикаторами */}
                                    {photos.length > 1 && (
                            ...
                            <div className='hidden md:flex justify-center mt-2 md:mt-0'>
                                <div className="bg-white/90 backdrop-blur-sm rounded-full shadow-lg p-1.5 md:p-2 flex gap-1 md:gap-2 z-10 items-center overflow-x-auto max-w-full">
                                    ...
                                    <div className="hidden md:flex w-px h-6 bg-gray-300 mx-0.5mx-1 shrink-0"></div>
